...
if (! function_exists('home_base_price_api')) {
    function home_base_price_api($id)
    {
        $product = Product::findOrFail($id);
        $price = $product->unit_price;
        if($product->tax_type == 'percent'){
...
if (! function_exists('home_base_price_api_v3')) {
    function home_base_price_api_v3($product)
    {
        $price = $product->unit_price;
        if($product->tax_type == 'percent'){
            $price += ($price*$product->tax)/100;
...
//Shows Base Price with discount
if (! function_exists('home_discounted_base_price_api')) {
    function home_discounted_base_price_api($id)
    {
        $product = Product::findOrFail($id);
        $price = $product->unit_price;
...
//Shows Base Price with discount
if (! function_exists('home_discounted_base_price_api_v3')) {
    function home_discounted_base_price_api_v3($product)
    {
        $price = $product->unit_price;
        $flash_deals = \App\Models\V3\FlashDeal::where('status', 1)->get();
        $inFlashDeal = false;
        foreach ($flash_deals as $flash_deal) {
            if ($flash_deal != null && $flash_deal->status == 1 && strtotime(date('d-m-Y')) >= $flash_deal->start_date && strtotime(date('d-m-Y')) <= $flash_deal->end_date && FlashDealProduct::where('flash_deal_id', $flash_deal->id)->where('product_id', $product->id)->first() != null) {
                $flash_deal_product = \App\Models\V3\FlashDealProduct::where('flash_deal_id', $flash_deal->id)->where('product_id', $product->id)->first();
                if($flash_deal_product->discount_type == 'percent'){
                    $price -= ($price*$flash_deal_product->discount)/100;
                }
                elseif($flash_deal_product->discount_type == 'amount'){
...
if (!function_exists('api_asset')) {
    function api_asset($id)
    {
        ...
        return URL::to('/') ."/public/uploads/all/zhE1QNuWDvkWoSO6s5bcK08rRF7fQlZ6utTAHSDN.jpg";
    }
}
...
if (!function_exists('uploaded_asset_nullable')) {
    function uploaded_asset_nullable($id)
    {
        if ($id) {
            $ids = explode(',',$id);
            if (($asset = \App\Upload::find($ids[0])) != null) {
...
if (!function_exists('uploaded_assets_nullable')) {
    function uploaded_assets_nullable($id)
    {
        ...
        if ($id) {
            ...
            array_push($photos,my_asset("/public/uploads/all/zhE1QNuWDvkWoSO6s5bcK08rRF7fQlZ6utTAHSDN.jpg"));
        }
        return $photos;
    }
}
...
if (! function_exists('homePriceApi')) {
    function homePriceApi($id)
    {
        $product = Product::findOrFail($id);
        $lowest_price = $product->unit_price;
        $highest_price = $product->unit_price;
        ...
        $lowest_price = convert_price_api($lowest_price);
        $highest_price = convert_price_api($highest_price);
        if($lowest_price == $highest_price ){
            return $lowest_price;
        }
